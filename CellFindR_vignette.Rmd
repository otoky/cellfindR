...
knitr::opts_knit$set(root.dir = "/Users/kyu/Desktop/02cochlea/TM_epi")
```
## CellFindR pipeline
This tutorial helps you run cellfindR on your dataset. Please make sure you have Seurat installed (preferably the latest Seurat 3 version and that you run all the functions that are included in the CellFindR.R script file. 
...
For this case, I will be demoing using Tympanic Membrane epithelium of the Mouse dataset from Frumm et al: https://pubmed.ncbi.nlm.nih.gov/33181078/
...
if you only have the barcodes, genes, matrix raw files, please run the seurat tutorial to make a Seurat object or use the load_tenx function provided in the CellfindR wrapper functions, if not skip the next section and load your seurat R data. 

```{r process r raw data}
## load data from processed RDS
file_loc <- "/Users/kyu/Desktop/02cochlea/TM_Epi"
#load_tenx(file_loc, res = 0.1, proj_name = 'TM_epi', cutoff= 10000, mito = FALSE)
```

```{r load data}
## load data from processed RDS
tenx <- readRDS(file = "/Users/kyu/Desktop/02cochlea/TM_Epi/TM_Epi.rds")
proj_name <- 'TM_epi'
```
...
#run cellfindR subclustering analysis
